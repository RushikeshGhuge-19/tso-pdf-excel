"""
TSO Converter  (v4.1 — FIXED for new PDF format)
========================================================
...
- Page 3 tool dimensions: 42-48 → 44-50 (NEW format offsets)
- Added PROC_RULES for: DRAW, TRIMMING, FLANGE, RESTRIKE, PART OFF
- Fixed blank thickness extraction (col 17 OLD → col 14 NEW, try both)
...
#
# FIX v4.1: Added DRAW, TRIM, FLANGE, RESTRIKE, PART OFF rules
#
PROC_RULES = [
        ...
        if len(pdf.pages) >= 3:
            ...
            for idx, row in enumerate(tbl):
                ...
                if rc and rc[0] == '1':
                    ...
                    blank_thk = ''
                    if len(rc) > 14 and rc[14]:
                        blank_thk = rc[14]
                    elif len(rc) > 17 and rc[17]:
                        blank_thk = rc[17]
            ...
            #      [47]=tonnage, [48]=press_type, [49]=parts_per, [50]=construct
            # OLD: [40]=name_part1, [41]=name_part2, [42]=L, [43]=W, [44]=H,
            ...
            def extract_op(rc, n1, n2, lc):
                ...
                u = name.upper()
                if not any(k in u for k in ['BLANK','FORM','PIERC','PUNCH','INSPECT','SHEAR','PANEL','WELD','TOOL','TAP','DRAW','TRIM','FLANGE','RESTRIKE','PART OFF']):
                    return None
                return {
                    'raw_name':  name,
            ...
            # Try NEW format first (42-43), fallback to OLD (40-41)
    ...
    if 'Inhouse RM' in wb.sheetnames:
        ...
        if ws.max_row >= 3:
            ...
            data['inhouse_rm'] = {
                ...
                'width':      cl(r3[10]) if len(r3) > 10 else '',   # Column 11 (Width)
                'height':     cl(r3[11]) if len(r3) > 11 else '',   # Column 12 (Height)
                'blank_thk':  cl(r3[12]) if len(r3) > 12 else '',   # Column 13 (Thickness)
                'od':         cl(r3[13]) if len(r3) > 13 else '',   # Column 14 (OD)
                'density':    cl(r3[14]) if len(r3) > 14 else '',   # Column 15 (Density)
                'input_wt':   cl(r3[15]) if len(r3) > 15 else '',   # Column 16 (Gross Value)
                'scrap_value': cl(r3[16]) if len(r3) > 16 else '',  # Column 17 (Scrap Value)
                'output_wt':  cl(r3[17]) if len(r3) > 17 else '',   # Column 18 (Net Value)
            }
...
def write_excel(data, template_path, out_path):
    ...
    for i, canon_pno in enumerate(tpl_row_order):
        ...
        if not ws.cell(r, 6).value:
            ...
            if type_part in ('BOU', 'BOP'):
                sv(ws, r, 6, 'BOP')
            elif type_part in ('STAMPED', 'INHOUSE','STAMPING PART'):
                sv(ws, r, 6, 'Inhouse')
    ...
    if rm_grade_val:
        ...
        # Only flag if it doesn't match Library exactly
    ...
    # Length, Width, Height, Thickness (cols 10-13)
    ...
    for op in tool_ops:
        ...
        # New ops
        if 'DRAW' in u and cats['DRAW'] is None:
            cats['DRAW'] = op
        elif 'TRIM' in u and cats['TRIM'] is None:
            cats['TRIM'] = op
        elif 'FLANGE' in u and cats['FLANGE'] is None:
            cats['FLANGE'] = op
        elif 'RESTRIKE' in u and cats['RESTRIKE'] is None:
            cats['RESTRIKE'] = op
        elif 'PART' in u and 'OFF' in u and cats['PART_OFF'] is None:
            cats['PART_OFF'] = op
        # Old ops
        elif 'BLANK' in u and 'FINE' not in u and 'PROFILE' not in u and cats['BLANK_PIERCE'] is None:
            cats['BLANK_PIERCE'] = op
        elif ('1ST' in u or 'FIRST' in u) and 'FORM' in u and cats['FORM_1'] is None:
            cats['FORM_1'] = op
        elif ('2ND' in u or 'SECOND' in u) and 'FORM' in u and cats['FORM_2'] is None:
            cats['FORM_2'] = op
    ...
    cat_pdf_kws = {
        ...
        'DRAW':         ['DRAW'],             # → 'Drawing'
        'TRIM':         ['TRIM'],             # → 'Trimming'
        'FLANGE':       ['FLANGE'],           # → 'Flanging'
        'RESTRIKE':     ['RESTRIKE'],         # → 'Restrike'
        'PART_OFF':     ['PART','OFF'],       # → 'Cut off'
    ...
    # ── Stamping ops (Blank, Draw, Trim, Flange, Restrike, Part-off, Forming, Piercing, Inspection) ───
    ...
    for cat_key in ordered_cats:
        ...
        # Strip 'T' suffix from tonnage (e.g. '250T' → '250')
...
# ── Footer ────────────────────────────────────────────────────────────────────
st.divider()
st.caption("TSO Converter v4.1 · FIX: NEW PDF format support (weights cols 34-35, tools cols 42-50, DRAW/TRIM/FLANGE/RESTRIKE/PART OFF rules)")

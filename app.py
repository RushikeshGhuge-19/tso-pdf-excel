...
def normalise_ftg_name(raw):
    ...
    Fixes: FORM→Forming, bare PIERCING→Piercing Tool, OCR garbage inspection."""
    if not raw:
        return ''
    u = raw.upper().strip()
    # OCR corruption: INSPECTION + garbled → Panel checker
    if u.startswith('INSPECTION') and len(u) > 12:
        return 'Panel checker'
    tc = title_case(raw)
    # 'Form Tool' → 'Forming Tool' (PDF abbreviates)
...
def write_excel(data, template_path, out_path):
    ...
    # Fallback: derive assy/child from BOM if template rows are blank
    ...
    # ── Write assembly-level ops rows 3-5 (always explicit, not relying on template) ──
    assy_ftg_weld = find_in_lib(['tool', 'supplier'], lib.get('X', []))
    ...
    assy_rows = [
        # (row, mfg, sub_rule, ftg, ftg_name, p1t, p1v, mach_make, mach_spec, remarks)
        (3, assy_mfg_oth, 'PROJECT|WELD', assy_ftg_weld, 'Projection welding pin', assy_p1t_str, 5,
         'Nash', 'MFDC ',
         'Shutter type PY for projection welding with 100% nut presence checking'),
        (4, assy_mfg_oth, 'RETAP',        assy_ftg_weld, 'M6 and M8 Tap',          assy_p1t_str, 5,
         '', '', ''),
        (5, assy_mfg_oth, 'INSPECT',      assy_ftg_gau,  'Assy checking Fixture',   assy_p1t_pcs, 1,
         '', '',
         'Shutter type PY for projection welding with 100% nut presence checking'),
    ]
    for (row_r, mfg_v, sub_rule, ftg_v, ftg_nm, p1t_v, p1_val, mach_mk, mach_sp, remarks) in assy_rows:
        _, sub_v, _, _, _, _, _ = match_rule(sub_rule, lib)
        # Write Level=0 and assy PartNo/Desc on EVERY assembly row
        sv(ws, row_r, 1, '0')
        ...
        if remarks: sv(ws, row_r, 19, remarks)
    ...
    # Shearing is level=0, uses assy part no (pre-stamping material prep at assembly level)
    sv(ws, cur, 1, '0'); sv(ws, cur, 2, assy_pno); sv(ws, cur, 3, assy_desc)

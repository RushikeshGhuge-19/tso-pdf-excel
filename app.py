    ...
    if 'Inhouse RM' in wb.sheetnames:
        ...
        if ws.max_row >= 3:
            ...
            data['inhouse_rm'] = {
                ...
                'blank_thk':   cl(r3[11]) if len(r3) > 11 else '',
                'od':          cl(r3[12]) if len(r3) > 12 else '',
                'density':     cl(r3[13]) if len(r3) > 13 else '',
                'input_wt':    cl(r3[15]) if len(r3) > 15 else '',  # col 16 = Gross
                'scrap_value': cl(r3[16]) if len(r3) > 16 else '',  # col 17 = Scrap
                'output_wt':   cl(r3[17]) if len(r3) > 17 else '',  # col 18 = Net
            }
...
def write_excel(data, template_path, out_path):
    ...
    if not ws.cell(r, 9).value  and rm_param: sv(ws, r, 9,  rm_param)
    if not ws.cell(r, 10).value and rm_uom:   sv(ws, r, 10, rm_uom)

    # Thickness (col 13)
    # FIX v4.2 BUG 3: use BOM thickness (page 2 col 9), not page-3 col 14
